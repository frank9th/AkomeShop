	...
	$(document).ready(function(){
		...
		**/
		...
		var a = document.getElementById('auto-save'); // element
		a.onclick = function(b) { 
			a.classList.add("text-primary");
			clicks = ++clicks

			if (clicks == 1 ){
				window.alert("Activating auto save ")
				console.log("activating auto save ")
				console.log(clicks)
				$("#save-on").show();
				$("#save-off").hide();
			
			
				localStorage.setItem('saveOn', 'clicks');
			};
			if (clicks > 1 ){	
				a.classList.remove("text-primary");
				$("#save-on").hide();
				$("#save-off").show();
				alert("do you want to disable auto save? ")
				console.log("do you want to disable auto save? "); 
			...
			if (clicks > 2 ){	
				...
				$("#save-on").show();
				$("#save-off").hide();
				window.alert("What do you want to do? ")
				...
				$.ajax({
					...
					success:function(data) {
							...
							y = data.payment_type
								...
								$("#tsmsg").text("99% completed, Just a minute. we need to confirm your cash. Ref_code: " + data.trans_code )
								$("#tsmsg").show()
							...
							y = data.payment_type
								...
								$("#tpForm-card").hide();
								//alert("topop successful")
								$("#tpmsg").text("Greate, kindly pay into; Acc. No: 113339999. Bank: UBA to complete transaction  Ref_code: " + data.trans_code  )
								$("#tpmsg").show()
							//alert("topop successful")
								/*
								TODO: SEND THIS TO CFO DASHBOARD TO CONFIRM PAY 
					...
					success:function(data) {
						if (data.status == "Save"){
							x = data.wallet_balance
								...
								$("#dform").hide();
							//alert("topop successful")
								$("#wsmsg").text(" You just requested for a cash out of " +  amount + " Ref_code:" + data.trans_code)
								$("#wsmsg").show();
								$("#wpmsg").hide();
						...
						success:function(data) {
							...
							if (data.status == 50 ){
								$("#sendform").show();
									...
									//alert("topop successful")
								$("#cfsmsg").text("Sending to " + rec_acc + " account " )
								$("#cfsmsg").show();
								$("#rec_act_no_Id").val(rec_acc);
								$("#rec_act_type_Id").val(rec_acc_type);
								//console.log(rec_acc_type)
					...
					success:function(data) {
						if (data.status == 200 ){
							...
							//alert("Send money successful")
								$("#sesmsg").text(" Transfered successful, Ref_code:" + data.trans_code )
								$("#sesmsg").show();
								//$("#sepmsg").hide();
	...
	event.preventDefault();
		...
		$("#sav-proceed").click(function(){
			console.log("Save Proceed button clicked ")
				...
				console.log("Enter Savings duration   ")
				alert("Enter Savings duration ")
			}
				else if ( pyd < 3){
				console.log("The minimum duration is 3 months  ")
				alert("whoops! The minimum duration is 3 months ")
			}
				samount ={
					pay_day: pyd,
					actUser: acctuser,
				...
				$.ajax({
					...
					success:function(data) {
						...
						if (data.status == 100 ){
							$("#savCard").show();
							$("#savForm-card").hide();
							$("#sdmsg").text("We are so sorry. the minimum saveings duration is 3 months. in other to qualify to earn 10% on our raffle draw ")
							$("#sdmsg").show();
							$("#smsg").hide();
							}
	...
	// Confirming Transaction funtion 

	$("#transTable").on("click", ".btn-action", function(){
		console.log("confirmed Button Clicked")
		//console.log("confirmed Button Clicked")
		let id = $(this).attr("data-sid");
		let code = $("#trns_refId").val(id);
		
		let user = $("#userId").val();
		let csrf = $("input[name=csrfmiddlewaretoken]").val();
		
		trans_data = {code:id, csrfmiddlewaretoken: csrf, user_id:user }
					...
					success:function(data) {
						if (data.status== 200){
							...
							for (i= 0; i < x.length; i++ ){
								output += "<tr><td>" + x[i].status + "</td><td>" + x[i].transaction_type + "</td><td>" + x[i].amount + "</td><td>" + x[i].note + "</td><td>" + x[i].time + "</td><td>" + x[i].ref_code + "</td><td><input type='button' name='action' value='Action' class='btn btn-warning btn-sm btn-action' data-sid=" + x[i].ref_code + ">"	
							}
							$("#tran_smsg").text("Transaction Completed")
							$("#tran_smsg").show();
							$("#transTable").html(output);
